...
def ensure_profile(user: dict) -> dict:
    ...
    if not isinstance(profile, dict):
        profile = {}
    profile.setdefault("display_name", user.get("username", ""))
...
def public_profile(user: dict) -> dict:
    ...
    return {
        "display_name": profile.get("display_name", user.get("username", "")),
...
<title>گپینو</title>
<style>
*{box-sizing:border-box}html,body{width:100%;height:100%;margin:0}body{font-family:Tahoma,Arial,sans-serif;background:#0e141b;color:#fff;direction:rtl}button,input,textarea{font-family:inherit}button{cursor:pointer}.hidden{display:none!important}
.auth{min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px;background:radial-gradient(circle at top,#1b3158,#0e141b 60%)}
.card{width:100%;max-width:420px;background:#171f29;border:1px solid #2d3947;border-radius:22px;padding:28px;box-shadow:0 25px 70px rgba(0,0,0,.45)}
.logo{text-align:center;font-size:38px;font-weight:900}.sub{text-align:center;color:#9aa7b7;margin:8px 0 24px}.card input{width:100%;margin-bottom:12px;padding:13px 14px;border-radius:12px;border:1px solid #354253;background:#0d131a;color:#fff;outline:none}.card input:focus{border-color:#3b82f6}
...
<body>
<div id="auth" class="auth"><div class="card"><div class="logo">گپینو</div><div class="sub">پیام‌رسان اینترنتی گپینو</div>
<div id="loginBox">
<input id="loginUser" placeholder="نام کاربری" autocomplete="username">
<input id="loginPass" type="password" placeholder="رمز عبور" autocomplete="current-password">
<button class="btn primary" onclick="loginUser()">ورود به گپینو</button>
<button class="btn secondary" onclick="showRegister()">ساخت حساب جدید</button>
</div>
<div id="registerBox" class="hidden">
<input id="regUser" placeholder="نام کاربری" autocomplete="username">
<input id="regPass" type="password" placeholder="رمز عبور" autocomplete="new-password">
<input id="regPass2" type="password" placeholder="تکرار رمز عبور" autocomplete="new-password">
<button class="btn primary" onclick="registerUser()">ساخت حساب</button>
<button class="btn secondary" onclick="showLogin()">بازگشت به ورود</button>
</div>
<div id="error" class="error"></div></div></div>
<div id="app" class="app hidden">
<aside id="sidebar" class="sidebar"><div class="side-head"><div class="side-logo">گپینو</div><div id="me" class="me"></div>
<input id="search" class="search" placeholder="جستجوی کاربران..." oninput="filterUsers()">
<div class="side-buttons"><button class="side-action" onclick="openProfile()">پروفایل من</button><button class="side-action" onclick="loadUsers()">به‌روزرسانی</button></div>
<button class="logout" onclick="logoutUser()">خروج از حساب</button></div>
...
<input id="profileAvatarFile" type="file" accept="image/jpeg,image/png,image/gif,image/webp" class="hidden" onchange="handleAvatarSelect(event)">
<div id="selectedFileName" class="file-name"></div>
<button class="remove-avatar" type="button" onclick="removeAvatar()">حذف عکس پروفایل</button>
<label for="profileDisplayName">نام نمایشی</label><input id="profileDisplayName" maxlength="50" placeholder="مثلاً امیر حسین">
<label for="profileStatus">وضعیت</label><textarea id="profileStatus" maxlength="150" placeholder="مثلاً در حال کار با گپینو هستم"></textarea>
<div class="modal-actions"><button class="primary" onclick="saveProfile()">ذخیره تغییرات</button><button class="secondary" onclick="closeProfile()">انصراف</button></div>
<div id="profileError" class="error"></div></div></div>
<script>
let me=null,users=[],selected=null,socket=null,typingTimer=null,reconnectTimer=null,selectedAvatarFile=null,removeAvatarFlag=false;
window.onlineUsers=[];
function err(text){document.getElementById("error").textContent=text||""}
function profileErr(text){document.getElementById("profileError").textContent=text||""}
function esc(value){return String(value??"").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}
function safe(value){return String(value).replace(/[^a-zA-Z0-9_-]/g,"_")}
async function api(url,options={}){const response=await fetch(url,{credentials:"include",...options});let data={};try{data=await response.json()}catch(_){}if(!response.ok)throw new Error(data.detail||"خطایی رخ داد.");return data}
function profileOf(user){return user&&user.profile||{}}
function userLabel(user){const p=profileOf(user);return p.display_name||user.username}
function userStatus(user){const p=profileOf(user);return p.status||""}
function userAvatarHtml(user){const p=profileOf(user);if(p.avatar)return '<img src="'+esc(p.avatar)+'" alt="">';const name=userLabel(user)||user.username||"?";return esc(name.substring(0,1).toUpperCase())}
function showLogin(){document.getElementById("loginBox").classList.remove("hidden");document.getElementById("registerBox").classList.add("hidden");err("")}
function showRegister(){document.getElementById("loginBox").classList.add("hidden");document.getElementById("registerBox").classList.remove("hidden");err("")}
async function loginUser(){const username=document.getElementById("loginUser").value.trim();const password=document.getElementById("loginPass").value;if(!username||!password){err("نام کاربری و رمز عبور را وارد کنید.");return}try{await api("/login",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:new URLSearchParams({username,password})});await startApp()}catch(e){err(e.message)}}
async function registerUser(){const username=document.getElementById("regUser").value.trim();const password=document.getElementById("regPass").value;const password2=document.getElementById("regPass2").value;if(!username||!password||!password2){err("همه فیلدها را پر کنید.");return}if(password.length<4){err("رمز عبور حداقل ۴ کاراکتر باشد.");return}if(password!==password2){err("تکرار رمز عبور صحیح نیست.");return}try{await api("/register",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:new URLSearchParams({username,password})});document.getElementById("loginUser").value=username;document.getElementById("loginPass").value=password;showLogin();err("حساب ساخته شد. اکنون وارد شوید.")}catch(e){err(e.message)}}
async function logoutUser(){try{await api("/logout",{method:"POST"})}catch(_){}if(socket){try{socket.close()}catch(_){}}clearTimeout(reconnectTimer);socket=null;me=null;selected=null;users=[];window.onlineUsers=[];document.getElementById("app").classList.add("hidden");document.getElementById("auth").classList.remove("hidden")}
async function startApp(){const data=await api("/me");if(!data.user)return;me=data.user;document.getElementById("auth").classList.add("hidden");document.getElementById("app").classList.remove("hidden");renderMe();await loadUsers();connectSocket()}
function renderMe(){document.getElementById("me").textContent="حساب: "+userLabel(me)}
async function loadUsers(){try{const data=await api("/users");users=data.users||[];renderUsers(filterList(users))}catch(e){console.error(e)}}
function filterList(list){const q=document.getElementById("search").value.trim().toLowerCase();if(!q)return list;return list.filter(u=>(u.username||"").toLowerCase().includes(q)||(userLabel(u)||"").toLowerCase().includes(q))}
function filterUsers(){renderUsers(filterList(users))}
function renderUsers(list){const box=document.getElementById("users");box.innerHTML="";if(!list.length){box.innerHTML='<div style="padding:20px;text-align:center;color:#788596;">کاربری پیدا نشد</div>';return}for(const user of list){const el=document.createElement("div");el.className="user"+(selected&&selected.username===user.username?" active":"");el.innerHTML='<div class="avatar">'+userAvatarHtml(user)+'</div><div class="uinfo"><div class="uname">'+esc(userLabel(user))+'</div><div class="ustatus" id="status-'+safe(user.username)+'">'+esc(userStatus(user)||user.username)+'</div></div>';el.onclick=()=>selectUser(user);box.appendChild(el)}updateStatuses()}
function selectUser(user){selected=user;document.getElementById("chatName").textContent=userLabel(user);document.getElementById("typing").textContent="";updateChatStatus();loadConversation();renderUsers(filterList(users));if(window.innerWidth<=800)document.getElementById("sidebar").classList.add("closed")}
function updateStatuses(){for(const user of users){const el=document.getElementById("status-"+safe(user.username));if(!el)continue;const online=window.onlineUsers.includes(user.username);el.textContent=online?"آنلاین":(userStatus(user)||user.username);el.className="ustatus"+(online?" online":"")}updateChatStatus()}
...
function previewCurrentAvatar(avatar,name){const box=document.getElementById("profileAvatar");if(avatar){box.innerHTML='<img src="'+esc(avatar)+'" alt="">';return}box.textContent=(name||me?.username||"؟").substring(0,1).toUpperCase()}
function handleAvatarSelect(event){const file=event.target.files&&event.target.files[0];if(!file)return;const allowed=["image/jpeg","image/png","image/gif","image/webp"];if(!allowed.includes(file.type)){profileErr("فقط JPG، PNG، GIF و WEBP مجاز است.");event.target.value="";return}if(file.size>5*1024*1024){profileErr("حجم عکس نباید بیشتر از ۵ مگابایت باشد.");event.target.value="";return}selectedAvatarFile=file;removeAvatarFlag=false;document.getElementById("selectedFileName").textContent=file.name;const reader=new FileReader();reader.onload=()=>{document.getElementById("profileAvatar").innerHTML='<img src="'+esc(reader.result)+'" alt="">'};reader.readAsDataURL(file);profileErr("")}
function removeAvatar(){selectedAvatarFile=null;removeAvatarFlag=true;document.getElementById("profileAvatarFile").value="";document.getElementById("selectedFileName").textContent="عکس پروفایل حذف خواهد شد";const name=document.getElementById("profileDisplayName").value.trim();document.getElementById("profileAvatar").textContent=(name||me?.username||"؟").substring(0,1).toUpperCase();profileErr("")}
function closeProfile(){document.getElementById("profileModal").classList.add("hidden");profileErr("")}
async function saveProfile(){const displayName=document.getElementById("profileDisplayName").value.trim();const status=document.getElementById("profileStatus").value.trim();if(!displayName){profileErr("نام نمایشی نمی‌تواند خالی باشد.");return}try{profileErr(selectedAvatarFile?"در حال آپلود عکس...":"در حال ذخیره...");if(selectedAvatarFile){const form=new FormData();form.append("file",selectedAvatarFile);const uploadData=await api("/profile/avatar",{method:"POST",body:form});me=uploadData.user}const payload={display_name:displayName,status:status};if(removeAvatarFlag)payload.avatar="";const data=await api("/profile",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});me=data.user;renderMe();await loadUsers();if(selected){const updated=users.find(u=>u.username===selected.username);if(updated){selected=updated;document.getElementById("chatName").textContent=userLabel(selected);updateChatStatus()}}closeProfile()}catch(e){profileErr(e.message)}}
document.getElementById("profileDisplayName").addEventListener("input",()=>{if(!selectedAvatarFile&&!removeAvatarFlag){const box=document.getElementById("profileAvatar");if(!box.querySelector("img")){const name=document.getElementById("profileDisplayName").value.trim();box.textContent=(name||me?.username||"؟").substring(0,1).toUpperCase()}}});
function toggleSidebar(){document.getElementById("sidebar").classList.toggle("closed")}
window.addEventListener("load",async()=>{try{const data=await api("/me");if(data.user)await startApp()}catch(_){} });
</script>
</body>
</html>'''
...
@app.get("/health")
async def health():
    return {"status": "ok", "app": APP_NAME, "time": now_text()}
...
async def avatar_file(filename: str):
    ...
    if clean_name != filename:
        raise HTTPException(404, "عکس پیدا نشد.")
    path = AVATARS_DIR / clean_name
    if not path.is_file():
        raise HTTPException(404, "عکس پیدا نشد.")
...
async def register(username: str = Form(...), password: str = Form(...)):
    ...
    if len(username) > 30:
        raise HTTPException(400, "نام کاربری نباید بیشتر از ۳۰ کاراکتر باشد.")
    if not username.replace("_", "").isalnum():
        raise HTTPException(400, "نام کاربری فقط شامل حروف انگلیسی، عدد و _ باشد.")
    if len(password) < 4:
    ...
    return {"ok": True, "message": "حساب با موفقیت ساخته شد."}
...
async def update_profile(request: Request):
    ...
    try:
        ...
        raise HTTPException(400, "اطلاعات پروفایل نامعتبر است.") from exc

    display_name = str(data.get("display_name", "")).strip()
    status = str(data.get("status", "")).strip()
    avatar_value = data.get("avatar", None)

    if not display_name:
        raise HTTPException(400, "نام نمایشی نمی‌تواند خالی باشد.")
    if len(display_name) > 50:
        raise HTTPException(400, "نام نمایشی نباید بیشتر از ۵۰ کاراکتر باشد.")
    if len(status) > 150:
        raise HTTPException(400, "وضعیت نباید بیشتر از ۱۵۰ کاراکتر باشد.")
    ...
    return {"ok": True, "profile": updated["profile"], "user": updated}
...
async def upload_profile_avatar(request: Request, file: UploadFile = File(...)):
    ...
    if content_type not in ALLOWED_AVATAR_TYPES:
        raise HTTPException(400, "فقط فایل‌های JPG، PNG، GIF و WEBP مجاز هستند.")

    content = await file.read()
    if not content:
        raise HTTPException(400, "فایل تصویر خالی است.")
    ...
    try:
        ...
        raise HTTPException(500, "ذخیره عکس روی سرور انجام نشد.") from exc
...
async def get_messages(request: Request, username: str):
    current = get_current_user(request)
    if not current:
        raise HTTPException(401, "ابتدا وارد حساب شوید.")
    if find_user(username) is None:
        raise HTTPException(404, "کاربر پیدا نشد.")
    return {"messages": get_private_messages(current["username"], username)}
...
async def send_to_user(username, payload):
    ...
    if dead:
        with connections_lock:
            group = connections.get(username)
...
async def broadcast_online_users():
    with connections_lock:
        sockets = [ws for group in connections.values() for ws in list(group)]
        online = list(connections.keys())
    payload = {"type": "online_users", "users": online}
    for websocket in sockets:
        try:
            await websocket.send_json(payload)
        except Exception:
...
async def store_message(sender: str, receiver: str, text: str):
    message = {
...
async def create_message(request: Request):
    ...
    try:
        ...
        raise HTTPException(400, "اطلاعات پیام نامعتبر است.") from exc

    receiver = str(data.get("receiver", "")).strip()
    text = str(data.get("text", "")).strip()

    if not receiver:
        raise HTTPException(400, "گیرنده مشخص نشده است.")
    if not text:
    ...
    message = await store_message(current["username"], receiver, text)
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    if not username or find_user(username) is None:
        await websocket.send_json({"type": "error", "message": "ابتدا وارد حساب شوید."})
    ...
    try:
        while True:
            ...
            if kind == "typing":
                ...
                if target and find_user(target) is not None:
                    await send_to_user(
                        target,
                        {
                            "type": "typing",
                ...
                if find_user(target) is None:
                    await websocket.send_json({"type": "error", "message": "کاربر پیدا نشد."})
                    continue

                message = await store_message(username, target, text)
                payload = {"type": "message", "message": message}
